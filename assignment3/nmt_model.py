...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        x_permuted = x.permute(1,2,0) #b,e,src_len
        x_conv = self.post_embed_cnn(x_permuted)
        x = x_conv.permute(2,0,1) #src_len,b,e

        x_packed = nn.utils.rnn.pack_padded_sequence(x, source_lengths)
        output_packed, (last_hidden, last_cell) = self.encoder(x_packed)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(output_packed) #src_len, b, 2h
        enc_hiddens = enc_hiddens.permute(1,0,2) #b,src_len, 2h

         # Concatenate forward and backward hidden states
        last_hidden_forward = last_hidden[0]  # (b, h)
        last_hidden_backward = last_hidden[1]  # (b, h)
        last_hidden_concat = torch.cat([last_hidden_forward, last_hidden_backward], dim=1)  # (b, 2h)

        last_cell_forward = last_cell[0]  # (b, h)
        last_cell_backward = last_cell[1]  # (b, h)
        last_cell_concat = torch.cat([last_cell_forward, last_cell_backward], dim=1)  # (b, 2h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim= 0):
            ...
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
             ...
             # Collect output and update previous output
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        new_dec_hidden, new_dec_cell = self.decoder(Ybar_t, (dec_state[0], dec_state[1]))

        # Create new decoder state tuple for return
        new_dec_state = (new_dec_hidden, new_dec_cell)

        dec_state = new_dec_state
        
        # (b, h) -> (b, 1, h)
        dec_hidden_expanded = new_dec_hidden.unsqueeze(1)

        # (b, src_len, h) -> (b, h, src_len)
        enc_hiddens_proj_transposed = enc_hiddens_proj.transpose(1, 2)
        
        e_t = torch.bmm(dec_hidden_expanded,enc_hiddens_proj_transposed).squeeze(1) # b,1,h x b,src_len, h -> (b,src_len)
        #move (b,h) to (b,1,h)
        #
        ...
        #(b,src_len) -> (b,1,src_len)
        alpha_t_expanded = alpha_t.unsqueeze(1)
        # (b,1,src_len) x (b,src_len,2h) -> (b,1,2h)
        attention_scores = torch.bmm(alpha_t_expanded, enc_hiddens)

        #(b,1,2h) -> (b,2h)
        a_t = attention_scores.squeeze(1)
